...
## 深度面板补 Open3D mesh 列 + 按接触取景(用户指出)
### 缺口
index 页的 depth_validation_panel.png 只有 raw / |dI| / **matplotlib 热力图**三列,
没有 mesh —— 而站上其他产物早已是 Open3D。用户给的参考图正是我们已有的渲染风格。
### 两处改动
1. 面板加第四列 `mesh_view(st["depth"])`, 与图库/工作台**共用同一渲染器**。
2. **按接触取景** `crop_to_contact` (o3d_view): 之前 mesh 里是 13.3mm 板面上
   一个 2mm 压痕, 读起来像"平板上的小点"。量化确认过**不是扁**:
   z_scale=1.6 后视觉纵横比已达 0.72-1.20(真实 0.45-0.75), 问题是取景不是缩放。
   裁到接触外扩 26px, 保留一圈平整凝胶作平面参照 —— 与参考图构图一致。
...
## 2026-08-07 · 用 GlowTact「自制 pad」重标定 LUT —— 结论: **不切换**, 前提被证伪
代码: `twm/force_recovery/glowtact_selfmade.py` (check|ref|modality|modality_pads|
calibrate|degeneracy|geom|feats|force|react|host|panel|calib_audit);
`lut_calibration.fit_sphere_geometry` 新增 `z0_bounds` 形参(默认 ±2 mm, 与旧行为逐位一致)。
产物(仅数据盘): `lut_calibration/glowtact_selfmade_lut.npz`, `lut_compare/*`。
**未改动**: `glowtact_lut.npz`、site 目录、任何已发布资产。

### 出发假设(用户提出)与它的检验
假设: React 传感器是 GlowTact 的**自制 pad**, 我们一直误用商用 GelSight 子集;
证据是静止胶体色相 React 90/82 vs 自制 62 vs 商用 169。
色相复核**属实**(我方独立测: 自制 H=62/S=97/V=160, 商用 176/102/129,
React 72 个 sensor-side 中值 76/96/96)。**但色相说的是胶体外观, 不是传感器原理。**

### found / evidence / fix / verified
| found | evidence | fix | verified |
|---|---|---|---|
| **自制 pad 不是光度立体传感器** —— 决定性 | 接触 dI 的 PCA 方差占比 (60 帧/pad, 覆盖整个深度带与整块 pad): 自制 **0.996/0.004/0.000 (秩1)**, 商用 0.648/0.314/0.038 (秩3); 色度占比 \|dI-mean_c(dI)\|/\|dI\| 自制 **0.197** vs 商用 0.791 | 光度 LUT 把颜色映到 **2 维梯度**, 秩1 信号是一个标量, **构造上无法表达方向** | 图 `lut_compare/modality_comparison.png`: 自制接触是均匀黑盘(R/G/B 同步 -144/-136/-100), 商用是标准三 LED 三瓣 |
| **React 属于商用那一类, 不属于自制那一类** | 72 个 sensor-side 全量: PCA 中值 **0.774/0.175/0.040**, 色度占比 **0.789** (范围 0.588-0.988) | 撤回"用错子集"的前提 | React 色度占比 0.789 ≈ 商用 0.791, 是自制 (0.197) 的 **4 倍** |
| 自制表在**自己家**就失败 | 自制 pad 120 张视野内球压, 与 dI 检出的接触圆(与表无关)比对: 梯度夹角 **72.4°** (随机 90°), 仅 25.1% 像素 <30° | —— 自标定救不了它 | 对照: 商用表在商用 pad **7.33°**/98.2%; Sparsh 自标定 4.5°/99.2% |
| 秩1 直接导致查找表退化 | 建表帧上 \|mean g\|/rms\|g\| (同色格内目标梯度的方向一致性): 商用 **0.931**, 自制 **0.213** | 同一颜色对应四面八方的坡度, 平均后互相抵消 | 77.5 万像素只落进 **4292** 个色格 (90³ 的 0.59%); 商用 126 万像素落进 193996 格 (26.6%) |
| **球拟合默认 z0 边界会静默产出空表** | 自制子集 z 基准比商用低 3.3 mm; 用默认 ±2 mm, z0 卡在边界=0, R²=-0.0000, LUT **0 个 bin** 却照常存盘 | `fit_sphere_geometry` 加 `z0_bounds`, 起点改为区间中点(默认区间中点仍是 0.0, 逐位不变); `calibrate` 打印 bin 数并对"解落在边界"告警 | 放宽到 (0,6): R=1.877 z0=4.121 n=462 R²=0.631 |
| 自制 pad 的球压数据本身**不满足球冠模型** | R 随 z 截断单调漂移 1.41→1.88, z0 1.55→4.12, R² 始终 ≤0.63; a 在 ~2.0 mm 饱和而 d 最深到 3.8 mm(压过赤道) | 只能报告, 不能修 | 商用 pad 同法 R²=0.948, d/R≤1.07 |
| **LUT 覆盖率再次被证明不是域外检测器** | 自制表在自制 pad 上"未标定色格"只有 **3.1%**(p90 4.8%), 却有 72° 方向错误 | 判据仍是方向误差 | 与 Sparsh 那轮结论一致 (覆盖 0.905-0.968 / 93.3°) |
| 参考帧质量不是瓶颈(本轮唯一的好消息) | 自制 pad 每族 50-100 张 initial: 分半 p95 = **1.0 灰度**, p99.9 = 2.0, max 3.0-4.0, 因参考噪声通过 \|dI\|>8 的像素 **0.000%** | 无需修 | 6 个族全查, 非抽样 |

### 几何验证(任务要求的两个量, 新旧同帧对照)
接触圆心一律取自 dI 的 `detect_circle`(与表无关), 解析球半径取**该 pad 自己拟合的 R**。
| 帧集 | 表 | 梯度夹角 | <30° | 剖面 RMS | 中心/峰值 dip | 峰深 | 未标定色格 |
|---|---|---|---|---|---|---|---|
| 自制 pad, n=120 | 旧(商用) | 90.44° | 16.5% | 73.9 µm | 0.733 | 0.854 mm | 58.7% |
| 自制 pad, n=120 | **新(自制)** | **72.35°** | 25.1% | 132.4 µm | 0.959 | 0.360 mm | 3.2% |
| 商用 pad, n=42 | **旧(商用)=宿主** | **7.33°** | 98.2% | 120.3 µm | 1.000 | 2.637 mm | 0.6% |
| 商用 pad, n=42 | 新(自制) | 79.47° | 12.2% | 73.7 µm | 0.803 | 0.787 mm | 97.4% |
⇒ 两张表**都**在自制 pad 上失败(90.4° / 72.4°), 商用表在商用 pad 上完好(7.3°)。
新表的 dip 0.959 与 RMS 73.7 µm 这类"好看"的数字是**幅度塌成 1/3 的副产物**, 不能单独读。

### React 上的域外程度(72 个 sensor-side, 每侧取 24 个最强接触帧, 共 1717 帧)
| 表 | 未标定色格 中位 | 均值 | p90 | 逐 side 范围 |
|---|---|---|---|---|
| 旧(商用) | **11.0%** | 13.1% | 24.2% | 2.7% – 31.6% |
| 新(自制) | **95.1%** | 94.4% | 98.2% | 89.5% – 99.3% |
宿主对照: 商用 pad+旧表 8.7%(p90 23.0%), 自制 pad+新表 3.1%(p90 4.8%)。
两张表对 React **同一批像素**的解码方向中值相差 **78.6°**(逐 side 55-121°) —— 近乎正交。
梯度夹角在 React 上无法算(没有已知球面), 故此处只报"表间分歧", 不当作准确度。
⇒ **换新表让 React 更域外, 不是更少**: 11% → 95%。

### 力评测(force_eval_all 同协议: 逐组半半分+isotonic, 5 seeds, 组内打乱对照)
`published` = ds_glowtact 原样口径(视野内判据来自**重建的**接触面积/质心, 因此两张表评的不是同一批帧);
`physical` = 只用机器人量(指令 x,y 在内区 + 压深 d=z−z0(pad) ∈[0.15,3.4] mm), 四格**同一批帧**。
| 口径 | pad | 表 | n | rho | MAE (N) | 组内打乱 | real−shuffle |
|---|---|---|---|---|---|---|---|
| published | 商用 | 旧 | 98 | **0.9852** | 0.202 | 0.063 | 0.92 |
| published | 商用 | 新 | 98 | 0.7905 | 1.671 | 0.165 | 0.63 |
| published | 自制 | 旧 | **0** | — | — | — | 无帧通过口径 |
| published | 自制 | 新 | 60 | 0.3444 | 2.862 | **0.360** | 0.02 |
| physical | 自制 | 旧 | 1034 | 0.6717 | 2.861 | 0.468 | 0.204 |
| physical | 自制 | **新** | 1034 | **0.5891** | 3.214 | 0.463 | **0.126** |
| physical | 商用 | 旧 | 1037 | 0.8585 | 1.934 | 0.167 | 0.692 |
| physical | 商用 | 新 | 1037 | 0.7745 | 2.672 | 0.153 | 0.622 |
**可比性边界**(必须写在结论前面):
1. 站上的 0.986 是**商用 pad**上的数字; 我用同一份代码复现为 **0.9852**(n=98, 站上 n=201),
   所以我的实现与已发布口径同源, 但**不同 pad 的数不能直接比大小** —— 压头、力程、z 基准、
   甚至 `round` 球的半径 (3.35 vs 1.88 mm) 都不同。
2. `published` 口径不是表无关的: 表若把光晕重建成大面积, 就自己把自己的帧筛掉(自制+旧表 n=0)。
   老老实实的对照只能看 `physical` 那四行。
3. `physical` 口径下商用+旧表只有 0.859 而非 0.986 —— 差额来自被边框裁切的接触,
   与站上"视野内 vs 全量"的既有结论同源, 不是本轮的退化。
4. 自制 pad 上组内打乱基线高达 0.46(商用只有 0.15-0.17), 因此自制那两行必须按
   real−shuffle 读: 旧表 0.204 / **新表 0.126**。新表在自己的 pad 上比外来表还差。

### React 深度产物(motherboard/2026-05-10/episode_000 left, 2010 个 fresh 帧)
两张表各自用**自己宿主 pad**重新拟合力模型后跑全 episode:
| 表 | 未标定色格中位 | 峰深 p99 | 力上限 | 最强 3 帧的行号 |
|---|---|---|---|---|
| 旧(商用) | 5.6% | 1.126 mm | 4.75 N | 4815 / 5101 / 3788 |
| 新(自制) | 91.7% | 0.944 mm | 7.56 N | 1506 / 3122 / 5892 |
两组"最强接触"**无一重合** —— 换表不是微调, 是换了一个完全不同的排序。
`lut_compare/depth_validation_panel_{old_commercial,new_selfmade}.png` 并排存档
(另存了当前站上已发布的那张作参照, 未覆盖任何已发布文件)。
**视觉陷阱记录**: 新表的深度图"更干净"(光晕裙边消失), 但原因不是物理更好, 而是
91.7% 的接触像素落在**空色格**, 最近邻填充给出接近零的梯度 —— 表没答案, 看起来像去噪。
不能凭这张图判定改善。

### 顺带发现(**未修**, 超出本任务范围且 showcase.py 在禁改清单里)
`showcase._glowtact_calib` 的权重拟合在 `feature_cache/lut_full.json` 上, 预测却喂
`debug_gallery.stages()` 的特征, 两者不是同一套:
- **单位**: 缓存的 `vol`/`area` 是**像素**单位(同一帧 area 6815 px, 而 stages() 报 10.86 mm²,
  折合 6301 px), `stages()` 是 mm²; 五个特征吃到该因子的不同次幂, isotonic 吸收不掉;
- **版本**: 同一帧 `maxd` 缓存 0.613 / 现行 0.468, 缓存早于当前重建。
症状: 端到端跑在**它自己的标定对象**上(GlowTact round, 视野内, 真值 0.19-4.75 N),
rho **0.143**, MAE **1.231 N**, 输出量程被压到 0.31-1.86 N; 同一模型形式用匹配特征重拟合
样本内 rho 0.997。**React 站上的牛顿值走的正是这条路径。**
可复现: `python -m force_recovery.glowtact_selfmade calib_audit`。需要用户决定是否重拟合(会动已发布数字)。

### 另一个未处置的量(记录以免下次重新踩)
用机器人指令 x,y 对接触质心做仿射回归, 实测像素尺度: 自制 16.9/16.2 px/mm,
商用 15.8/14.9 px/mm ⇒ 0.059-0.067 mm/px, 而 `MM_PER_PIXEL` 假设 **0.0415**(约 1.5 倍差)。
两块 pad 都是这样, 所以它是一个**全局尺度**, 对方向类指标(夹角)不变, 对绝对 mm 有影响。
未改: 动它会让所有已发布的 mm 数字失效, 且与本任务无关。

### 结论: **不应该切换到自制 pad 的表**
1. 前提被证伪 —— React 的接触 dI 是秩3 彩色(色度占比 0.789), 与商用 GelSight Mini
   (0.791) 同类, 与自制 pad (0.197) 不同类。色相相近是**胶体外观**的巧合。
2. 自制表在**自己的 pad 上**就只有 72.4°/25% (随机 90°), 不是"迁移不好", 是**这块 pad 上
   不存在可标定的光度映射**: 接触是均匀遮光, 一个标量无法编码二维坡度方向。
3. 换表使 React 的未标定色格 11% → 95%, 解码方向偏转 78.6°, 力评测在任何口径下都变差。
4. 唯一"变好"的是几张深度图的观感, 而那是空表造成的假象。
**保留 `glowtact_lut.npz`。** 真要提升 React, 正确路径是**在 React 自己的传感器上做一次球压
自标定**(Sparsh 那轮已证明可行: 93.3°→4.5°, 视野内 rho 0.878→0.968), 而不是换一块别人的 pad。

## React 力标定修复: 三条路径合一(用户确认后执行)
### 根因: 单位失配 + 三条独立路径
实测 lut_full.json(标定权重所用) vs stages()(推理所用):
area 16874 px² vs 22 mm² (~580x); vol ~1000x; **但 maxd 两边都是 mm(1.23 vs 1.09)**
=> **部分失配**, 产出的是"看着合理的错数"而非明显爆炸。
三条独立路径, 各自的端到端表现(同一批 GlowTact round, 真值已知):
| 路径 | 用途 | rho | 量程 |
|---|---|---|---|
| A showcase._glowtact_calib | 站点/视频/action_trace | **0.143** | 0.31-1.86 (真值 0.19-4.75) |
| B run_episode(v1 MLP × 单一 scale) | **87个npz -> 预览圆点 + 数据集列** | **0.297** | **0.01-103** (真值 0.16-8) |
| C force_eval_all | 站上评测数字 | 已验证 | — |
### 修法: 标定与推理必须同源
新建 `force_recovery/react_calib.py`: 用**当前 stages()** 重算 GlowTact round
的标定特征(471 帧, mm 单位), 而非改一个换算系数 —— 系数会再次漂移。
按**按压位置**留出保留集(相邻帧是近重复, 随机划分等于自评)。
增益场 u(x,y) 保留。
**裁切修正取代视野内过滤**: 0-8N 段只有 10% 完全在视野内(边界余量中位 -31px),
过滤只剩 48 帧。把裁切比例作为特征, 保留集实测:
0.607(基线) -> 0.712(边界余量) -> **0.739(裁切比例)**, MAE 1.73 -> **1.23N**,
且低端量程从 1.32 恢复到 **0.44N**(真值下限 0.16)。打乱对照 +0.096。
### 已接入
- showcase._glowtact_calib 改为委派 react_calib(站点/视频/action_trace)
- run_episode 改用 stages() + react_calib, 弃用 v1 DepthForceEstimator
  (顺带修了两处遗漏: kept_depths 仍调用 est; meta 里的 est 阈值字段)
- **PIPELINE_VERSION 3 -> 4**: 我改了标定却没提版本, 首次重算 72 个只重写了 1 个
  —— 版本戳机制正是防这个, 但它只在人记得提版本时有效。已提版本并全量重算。
单 episode 复核: 力 p50 4.07 / p95 6.26 / max 7.28 N(旧路径同段 0.01-103N)。
