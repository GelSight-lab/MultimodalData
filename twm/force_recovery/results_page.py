...
def dataset_figures() -> dict:
    ...
    datasets = {
        ...
        "glowtact": {
            "title": "Markerless gel (cnc_mini_26, 0-20 N): supervision buys the "
...
demonstrated. Reading the table: cnc and cnc_mini_26 sit ~0.9 above their
...
读表方式：cnc 与 cnc_mini_26 高出各自对照约 0.9；FEATS 高出 0.78，而它的对照平在 0.00。</p>
...
markerless cnc_mini_26 and cnc references, where the step is a bit-exact no-op.</p>
...
0 个误检，阈值从 3 到 16 灰阶都稳定；而在无 marker 的 cnc_mini_26 与 cnc 参考帧上恰好检出
...
<tr><td>cnc_mini_26 (markerless)</td><td>@@O_GLOW@@</td>
...
<h2>cnc_mini_26 — markerless gel, cleaned</h2>
<div class="card"><img src="assets/results_glowtact.png" alt="cnc_mini_26 panels">
...
Our cnc_mini_26 table applied to this foreign sensor reaches &rho;=0.878 on
...
table fails on cnc_mini_26 frames too, so this is a per-sensor property, not a
bad table.</p>
...
数据集质量在行内被控制,面板之间的差异就是方法之间的差异。</p>"""),
    ...
    ("(in view)</td>", "(视野内)</td>"),
    ("<td>cnc_mini_26 (markerless)</td>", "<td>cnc_mini_26(无点)</td>"),
    ("""<p>The pattern is narrower than we first wrote it. <b>FEATS collapses outside
its own gel domain</b> (0.96 → 0.04–0.07), and the physics pipeline does work on
every dataset here (@@RANGE@@) without ever seeing training data. But
<b>FeelAnyForce does not collapse</b>: it holds 0.83 / 0.90 / 0.97 across the
...
FeelAnyForce 非边缘达 0.92。</p>"""),
    ("<h2>cnc_mini_26 — markerless gel, cleaned</h2>", "<h2>cnc_mini_26——无点 gel,清洗过</h2>"),
    ("""<p class="footnote">Friendliest ground truth (centred presses, 10 free frames,
0–20 N). Per-indenter, calibrated within each family under the physical scope
(contact fully in view, gel not bottomed out), ρ is 0.975–0.992 across all six
indenters with MAE ≤ 0.73 N. Caveat: that calibration is refit per family, so
