"""Build processed/mode2_v1/ — episode .pt files sliced into contiguous
clean segments at every bad-frames boundary. Parallel across episodes.

Each output `.pt`:
  - same schema as mode1_v1 (view, tactile_left/right, sensor_*_pose,
    timestamps, tactile_*_intensity/area/mixed)
  - `_contact_meta` is extended with segmentation metadata (source_episode,
    source_segment_idx, source_h5_frame_range, etc.)

Also writes `segments.json` at repo root with a flat manifest.
"""
...
OUT_ROOT = Path("/media/yxma/Disk1/twm/processed/mode2_v1/motherboard")
BAD_FRAMES_PATH = Path("/media/yxma/Disk1/twm/figures/dataset_figures/bad_frames.json")
SEGMENTS_MANIFEST = Path("/tmp/segments.json")

MIN_SEGMENT_FRAMES = 16          # drop fragments too short to host any useful window
TASK = "motherboard"
TACTILE_THRESHOLD = 0.4          # for the contact_pct stat in segments.json
...
def process_episode(args):
    """Worker: load one .pt, slice into clean segments, save each to mode2_v1/."""
    ...
    for s_a, s_b in segments:
        ...
        # Drop slow-changing fields we don't reuse to avoid bloating each seg file
        # (kept: drift, ref images, etc.)

        # Slice every per-frame tensor
        new_ep: dict = {}
        for k, v in ep.items():
        ...
        torch.save(new_ep, out_pt)

        # Stats for segments.json
        mL = new_ep[f"tactile_left_mixed"].numpy()
        mR = new_ep[f"tactile_right_mixed"].numpy()
        contact_L = (mL > TACTILE_THRESHOLD).mean() * 100
        contact_R = (mR > TACTILE_THRESHOLD).mean() * 100
        ...
        written.append({
            ...
            "contact_pct_left":     round(float(contact_L), 2),
            "contact_pct_right":    round(float(contact_R), 2),
...
def main():
    bad = json.loads(BAD_FRAMES_PATH.read_text())
    ...
    print(f"Processing {len(work)} episodes with 4 workers...")

    OUT_ROOT.mkdir(parents=True, exist_ok=True)
    # Wipe any prior mode2_v1 build
    ...
    # Per-episode active-sensors lookup from bad_frames or tasks.json
    try:
        import urllib.request
        t = json.loads(urllib.request.urlopen("https://huggingface.co/datasets/yxma/React/raw/main/tasks.json").read())
        per_date_active = {
    ...
    manifest = {
        ...
        "description": (
            ...
            "quality filter. Use `source_h5_frame_range` to map back to "
            "the original H5 recording."
        ),
    ...
    SEGMENTS_MANIFEST.write_text(json.dumps(manifest, indent=2))
    print(f"\nWrote {len(all_segments)} segments  →  {total_frames:,} frames "
          f"({total_min:.1f} min). Manifest at {SEGMENTS_MANIFEST}.")
