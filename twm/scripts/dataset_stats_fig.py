...
def panel(d: dict, ceiling: float, path: Path, title: str) -> None:
    ...
    if nof:
        ax[1].text(.03, .06, "no force channel: " + ", ".join(nof),
    ...
    xa = np.arange(len(ts))
    ax[3].bar(xa - .16, [d[t]["new_pct"] for t in ts], .3,
              color=[COL[t] for t in ts], label="new tactile frames")
    xf = np.array([ts.index(t) for t in fts], float)
    if len(xf):
        ax[3].bar(xf + .16, [d[t]["sat_pct"] for t in fts], .3,
                  color=[COL[t] for t in fts], alpha=.42, hatch="///",
